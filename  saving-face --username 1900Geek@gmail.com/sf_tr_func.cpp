...
namespace SF
{
	...
	{
		SF_SCALAR tx[9] = {1,0,0,0,cos(ypr.yaw),-sin(ypr.yaw),0,sin(ypr.yaw),cos(ypr.yaw)};
		...
		SF_SCALAR temp[9] = {tx[0] * ty[0] + tx[1]*ty[3] + tx[2] * ty[6], tx[0] * ty[1] + tx[1]*ty[4] + tx[2] * ty[7], tx[0] * ty[2] + tx[1]*ty[5] + tx[2] * ty[8],
						     tx[3] * ty[0] + tx[4]*ty[3] + tx[5] * ty[6], tx[3] * ty[1] + tx[3]*ty[3] + tx[5] * ty[7], tx[3] * ty[2] + tx[4]*ty[5] + tx[5] * ty[8],
		...
		SF_SCALAR tmMatrix[9] = {temp[0] * tz[0] + temp[1]*tz[3] + temp[2] * tz[6], temp[0] * tz[1] + temp[1]*tz[4] + temp[2] * tz[7], temp[0] * tz[2] + temp[1]*tz[5] + temp[2] * tz[8],
									temp[3] * tz[0] + temp[4]*tz[3] + temp[5] * tz[6], temp[3] * tz[1] + temp[3]*tz[3] + temp[5] * tz[7], temp[3] * tz[2] + temp[4]*tz[5] + temp[5] * tz[8],
									temp[6] * tz[0] + temp[7]*tz[3] + temp[8] * tz[6], temp[6] * tz[1] + temp[7]*tz[4] + temp[8] * tz[7], temp[6] * tz[2] + temp[7]*tz[5] + temp[8] * tz[8]};

		SF_TR_MATRIX tm = {0};

		tm.trXYZ.x -= rwcReference.x;
		tm.trXYZ.y -= rwcReference.y;
		tm.trXYZ.z -= rwcReference.z;

		memcpy(tm.trMatrix, tmMatrix, 9);
